...
/* MODIFIED to update the drag handle with a formatted version of the current value */
/* MODIFIED Line 144, bugfix: window event handler used where document was wanted. Fixes issues with slider response when placed in dynamic layout context. */
...
Slider.prototype =
{
	...
	feedback: function()
	{
		...
		if(value != this.value.prev)
		{
			...
			if(value == 0.0)
				this.handle.innerHTML = 'None';
			else if(value == 1.0)
				this.handle.innerHTML = 'Full';
			else	
				this.handle.innerHTML = this.toFixed(value.toString(), 2).toString();
		}
	},
